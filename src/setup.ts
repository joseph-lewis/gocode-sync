...
// OpenCode → `~/.config/opencode/plugin/gocode-sync.js` (a `session.idle`
//            plugin — OpenCode has no hooks-file command array; a plugin
//            subscribes to the idle event, fetches the CURRENT session's
//            messages via OpenCode's own SDK `client`, and pipes them to
//            `gocode-sync on-sync --source opencode --stdin`).
//
...
/** OpenCode plugin's shell-out: the current session's messages arrive on stdin. */
export const OPENCODE_SYNC_COMMAND =
  "gocode-sync on-sync --source opencode --stdin || true";
...
/** Markers identifying OUR opencode plugin file (both must be present). */
const OPENCODE_MARKERS = ["gocode-sync", "--source opencode"] as const;

/** OpenCode config dirs we probe (primary first), mirroring @trygocode/notify. */
const OPENCODE_DETECT_DIRS = [".config/opencode", ".opencode"] as const;

/**
 * The OpenCode `session.idle` plugin we install. On idle it:
 *   1. reads ONLY the idle session's messages via OpenCode's own SDK
 *      (`client.session.messages({ path: { id } })`) — never lists or reads
 *      other sessions, and never touches the on-disk store directly;
 *   2. builds a tiny `{ session_id, workspace_path, title, messages }` payload;
 *   3. pipes it to `gocode-sync on-sync --source opencode --stdin`, which does
 *      ALL gating / redaction / size-cap / upload in the audited package.
 * It NEVER blocks the session: the child is detached + unref'd, stdio is wired
 * only for the stdin pipe, and every error is swallowed. If sync is disabled
 * server-side, the CLI no-ops (fail-closed) — the plugin doesn't decide that.
 */
export const OPENCODE_PLUGIN_CONTENT = `// gocode-sync — OpenCode session.idle chat-sync plugin (auto-generated by @trygocode/sync).
// Syncs ONLY the conversation that just went idle to your GoCode app, by
// fetching that one session's messages via OpenCode's SDK and piping them to
// the audited \`gocode-sync\` CLI. Uploads nothing unless you enabled
// "Sync my IDE chat" in the GoCode app (off by default). Never blocks the
// session: detached child, stdio ignored except stdin, errors swallowed.
//
// Stable markers (do not edit): gocode-sync --source opencode
// Managed by @trygocode/sync — \`npx @trygocode/sync uninstall\` removes this file.
import { spawn } from "node:child_process";

export const GocodeSync = async ({ client, directory, worktree } = {}) => ({
  event: async ({ event } = {}) => {
    try {
      ...
      // Fetch ONLY this session's messages (current session only).
      let messages = [];
      try {
        const res = await client.session.messages({ path: { id: sessionID } });
        messages = (res && res.data) ? res.data : (Array.isArray(res) ? res : []);
      } catch {
        return; // SDK shape changed / not reachable → nothing to sync
      }
      if (!messages || messages.length === 0) return;

      const payload = JSON.stringify({
        session_id: sessionID,
        workspace_path: worktree || directory || process.cwd(),
        messages,
      });

      const child = spawn(
        ${JSON.stringify(OPENCODE_SYNC_COMMAND)},
        { shell: true, detached: true, stdio: ["pipe", "ignore", "ignore"] },
      );
      child.on("error", () => {});
      if (child.stdin) {
      ...
      // never block the session on a sync failure
    }
  },
});
`;
...
/**
 * Resolve the OpenCode config dir: the first of {@link OPENCODE_DETECT_DIRS}
 * that already exists under HOME, else the primary one (created on write).
 */
async function resolveOpenCodeDir(opts?: PathOpts): Promise<string> {
...
async function opencodeConfigPath(opts?: PathOpts): Promise<string> {
  return path.join(await resolveOpenCodeDir(opts), "opencode.json");
}
async function opencodePluginPath(opts?: PathOpts): Promise<string> {
  return path.join(await resolveOpenCodeDir(opts), "plugin", "gocode-sync.js");
}

/** OpenCode's MCP entry shape differs from Cursor/Claude (a `mcp` map). */
const OPENCODE_MCP_ENTRY = {
  type: "local",
  command: ["npx", "-y", "@trygocode/sync", "mcp"],
  enabled: true,
} as const;

/** Merge our MCP entry into OpenCode's `mcp` map. Mutates in place. */
function mergeOpenCodeMcp(config: Record<string, unknown>): void {
...
/** Remove our MCP entry from OpenCode's `mcp` map. Returns true if changed. */
function stripOpenCodeMcp(config: Record<string, unknown>): boolean {
  if (isRecord(config.mcp) && MCP_SERVER_NAME in config.mcp) {
...
export async function setupSync(opts?: PathOpts): Promise<SetupResult> {
  ...
  try {
    ...
    // OpenCode — a session.idle plugin file + an MCP entry in opencode.json.
    // Only configure if the user actually uses OpenCode (a config dir exists),
    // so we never create ~/.config/opencode for a non-OpenCode user.
    try {
      const ocDir = await resolveOpenCodeDir(opts);
      const dirExists = await fs
        .stat(ocDir)
        .then(() => true)
        .catch(() => false);
      if (dirExists) {
        const pluginFile = await opencodePluginPath(opts);
        await fs.mkdir(path.dirname(pluginFile), { recursive: true });
        await fs.writeFile(pluginFile, OPENCODE_PLUGIN_CONTENT);
        paths.push(pluginFile);
        detail.push("OpenCode: wrote session.idle sync plugin");

        const ocConfig = await opencodeConfigPath(opts);
        const cfg = (await readJsonObject(ocConfig)) ?? {};
        mergeOpenCodeMcp(cfg);
        await writeJsonFile(ocConfig, cfg);
        paths.push(ocConfig);
        detail.push("OpenCode: merged MCP server entry");
      } else {
        detail.push("OpenCode: skipped (no ~/.config/opencode or ~/.opencode)");
      }
    } catch (err) {
      detail.push(`OpenCode: skipped (${errMessage(err)})`);
    }
...
export async function uninstallSync(opts?: PathOpts): Promise<SetupResult> {
  ...
  try {
    ...
    // OpenCode — remove our plugin file (only if it's ours) + MCP entry.
    const pluginFile = await opencodePluginPath(opts);
    const pluginRaw = await fs.readFile(pluginFile, "utf8").catch(() => null);
    if (pluginRaw && OPENCODE_MARKERS.every((m) => pluginRaw.includes(m))) {
      await fs.rm(pluginFile, { force: true });
      paths.push(pluginFile);
      detail.push("OpenCode: removed session.idle sync plugin");
    }
    const ocConfig = await opencodeConfigPath(opts);
    const ocCfg = await readJsonObject(ocConfig).catch(() => null);
    if (ocCfg && stripOpenCodeMcp(ocCfg)) {
      await writeJsonFile(ocConfig, ocCfg);
      paths.push(ocConfig);
      detail.push("OpenCode: removed MCP server entry");
    }
